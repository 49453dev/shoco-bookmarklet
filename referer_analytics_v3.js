(function() {
    ...
    async function startApp() {
        try {
                ...
                try {
                    ...
                    if (rankRows.title) {
                        rankRows.title.forEach((t, i) => {
                            const rawUri = rankRows.uri[i] || '';
                            const postNo = rawUri.match(/\d+$/)?.[0] || '';
                            // 조회된 realBlogId를 사용하여 링크 생성
                            const finalUrl = 'https://blog.naver.com/' + realBlogId + '/' + postNo;
                            rHtml += '<div class="post-item"><div class="post-rank">' + (i + 1) + '</div><div class="post-info"><a href="' + finalUrl + '" target="_blank" class="post-title">' + t + '</a><div class="post-meta"><span class="post-cv" style="color:#00ff95">' + rankRows.cv[i].toLocaleString() + ' CV</span> | <span>' + rankRows.createDate[i] + '</span></div></div></div>';
                        });
                    }
                    ...
                    const engines = totalData.referrerSearchEngine || [];
                    const shares = totalData.cv_p || [];
                    const counts = totalData.cv || [];

                    doc.getElementById('btn-area').innerHTML = '';
                    doc.getElementById('detail-area').innerHTML = '';

                    // 3. 병목 현상 방지를 위해 순차적 요청 및 화면 추가 (append)
                    for (let i = 0; i < domains.length; i++) {
                        try {
                            const dRes = await fetch('https://blog.stat.naver.com/api/blog/daily/referer/total/detail?timeDimension=DATE&additionalTimeDimension=WEEK&startDate=' + date + '&searchEngine=' + engines[i] + '&refererDomain=' + encodeURIComponent(domains[i]) + '&size=100&offset=0').then(r => r.json());
                            const rows = dRes?.result?.statDataList?.[0]?.data?.rows || {};
                            const bColor = borderColors[i % 5];
                            
                            const btn = doc.createElement('button');
                            btn.className = 'channel-btn';
                            btn.style = 'border-left:4px solid ' + bColor;
                            btn.onmouseover = function() { this.style.boxShadow = '0 0 10px ' + bColor; };
                            btn.onmouseout = function() { this.style.boxShadow = 'none'; };
                            btn.innerHTML = '<span class="btn-name">' + domains[i] + '</span><div class="btn-meta"><span class="btn-share">' + (shares[i] || 0).toFixed(1) + '%</span><span class="cv-text">' + (counts[i] || 0).toLocaleString() + ' CV</span></div>';
                            
                            const card = doc.createElement('div');
                            card.className = 'channel-card';
                            card.id = 'card-' + i;
                            card.style = 'border-top:2px solid ' + bColor;
                            
                            let rowsHtml = '';
                            const rCv = rows.cv || [];
                            const rUrls = rows.referrerUrl || [];
                            const rQueries = rows.searchQuery || [];
                            for (let j = 0; j < rCv.length; j++) {
                                rowsHtml += '<tr><td class="rank-td">' + (j + 1) + '</td><td class="label-cell"><a href="' + (rUrls[j] || '#') + '" target="_blank">' + simplifyUrl(rUrls[j], rQueries[j]) + '</a></td><td class="cv-cell">' + rCv[j].toLocaleString() + '</td></tr>';
                            }
                            ...
                            doc.getElementById('btn-area').appendChild(btn);
                            doc.getElementById('detail-area').appendChild(card);
            ...
            doc.getElementById('prevDay').onclick = function() {
                let d = new Date(doc.getElementById('datePicker').value);
                d.setDate(d.getDate() - 1);
                const s = d.toISOString().split('T')[0];
                doc.getElementById('datePicker').value = s;
                fetchData(s);
            };
            doc.getElementById('nextDay').onclick = function() {
                let d = new Date(doc.getElementById('datePicker').value);
                d.setDate(d.getDate() + 1);
                const s = d.toISOString().split('T')[0];
                doc.getElementById('datePicker').value = s;
            ...
            doc.getElementById('goToday').onclick = function() {
                doc.getElementById('datePicker').value = todayStr;
                fetchData(todayStr);
            };
            doc.getElementById('datePicker').onchange = function(e) {
