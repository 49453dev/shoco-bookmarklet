(async function() {
    ...
    if (!location.hostname.includes('stat.naver.com')) {
        if (confirm('네이버 통계 페이지에서만 작동합니다. 이동하시겠습니까?')) {
            location.href = targetUrl;
        }
        return;
    }

    // 2. 브라우저 차단 방지를 위해 팝업창부터 즉시 생성
    const newWin = window.open('', '_blank');
    if (!newWin) {
        alert('팝업이 차단되었습니다! 주소창 우측의 팝업 차단 해제 후 다시 시도해주세요.');
        return;
    }
    ...
    try {
        ...
        async function fetchData(date) {
            ...
            } catch (e) { console.error(e); if(doc.getElementById('loading')) doc.getElementById('loading').innerText = 'Data Error!'; }
