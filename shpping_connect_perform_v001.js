(async function() {
  ...
  const css = `
    ...
    h1{text-align:center; font-size:36px; margin:0 0 25px 0; letter-spacing:-0.5px; color:#fff; font-weight:200; text-transform: uppercase;}
    h1 span {color:${brandColor}; font-weight:600; text-shadow: 0 0 15px rgba(255, 131, 41, 0.4);}
    ...
    .amt{font-size:13px;font-weight:300;color:#00ff95;text-align:right;}
    .unit{font-size:10px; opacity:0.7; margin-right:1px;}
    .amt.minus{color:#ff3b30;}
    ...
    #loadBtn{padding:10px 20px; background:${brandColor}; color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:600;}
  `;
    ...
    <script>
      ...
      function generateMonthHtml(year, month, dataMap) {
        ...
        for(let day=1; day<=last.getDate(); day++) {
          ...
          h += '<div class="d ' + dayType + '"><div class="date">' + day + '</div>' + amtHtml + '</div>';
        }
        return h + '</div></div>';
      }
