(async function() {
    ...
    try {
        ...
        async function fetchData(date) {
            ...
            if (loader) { loader.style.display = 'block'; loader.innerText = 'Syncing...'; }
            try {
                ...
                for (let i = 0; i < domains.length; i++) {
                    ...
                    for (let j = 0; j < rCv.length; j++) {
                        rowsHtml += `<tr><td class="rank-td">${j + 1}</td><td class="label-cell"><a href="${rows.referrerUrl[j] || '#'}" target="_blank">${simplifyUrl(rows.referrerUrl[j], rows.searchQuery[j])}</a></td><td class="cv-cell">${rCv[j].toLocaleString()}</td></tr>`;
                    }
