javascript:(function(){
    ...
    async function start(){
        ...
        let h1=`<html><head><meta charset="UTF-8"><title>Category Report</title><style>${CSS_CAT}</style></head><body><h2>CATEGORY TREND REPORT</h2><div class="dt">${dt}</div>${EE}<div class="sec-t">Subject Categories</div><div class="w">${gen(d1)}</div><div class="sec-t">Demographic Categories</div><div class="w">${gen(d2)}</div></body></html>`;
        ...
        let h2=`<html><head><meta charset="UTF-8"><title>Top Posting</title><style>${CSS_HOME}</style></head><body><h2>Naver main TOP POSTING LIST</h2><div class="dt">기준일: ${dt}</div>${EE}<div class="w">`;
        for(let t=0;t<2;t++){
